...
{
    private const string TmdbProviderKey = nameof(MetadataProvider.Tmdb);
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(dirName) &&
                (dirName.StartsWith("Season", StringComparison.OrdinalIgnoreCase) ||
                 ...
                 (dirName.StartsWith("S", StringComparison.OrdinalIgnoreCase) && dirName.Length <= 4 && char.IsDigit(dirName[^1]))))
            {
        ...
        ILogger? logger)
    {
        ...
        seriesProviderIds.TryGetValue(TmdbProviderKey, out string? rawTmdbId);
